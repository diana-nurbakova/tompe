...
## Implementation sprint #8 — Longitudinal analytics + BKT (B4 + B5)

**Date range:** 2026-05-20
**Theme:** Replace the `NotImplementedError` stubs in `analytics.py` and `progression.py` with a working Bayesian Knowledge Tracing tracker, per-skill mastery probabilities, blind-spot detection, and BKT-gated progression badges.

### What landed

| ID | Item | Status | Files touched |
|---|---|---|---|
| B5 / BKT core | New `tompe.services.bkt` module with parameters (`p_init=0.10`, `p_transit=0.10`, `p_slip=0.10`, `p_guess=0.20`), `bkt_step` (Bayes + transition), `update_skill` / `update_from_scoring`, `get_mastery`, `bkt_skill_profile`. Per-student JSON store at `data/bkt/{student_id}.json`. | Done (unit-tested) | [src/tompe/services/bkt.py](../src/tompe/services/bkt.py), [src/tompe/schemas/scoring.py](../src/tompe/schemas/scoring.py) (new `BKTSkillState` + `StudentBKT`), [src/tompe/services/datastore.py](../src/tompe/services/datastore.py) (new `bkt_store` + `profiles_store`) |
| B4 / analytics | `update_student_profile` folds new ScoringResults into per-MQM and per-ToM `PerformanceTimeSeries`; trend label = OLS slope on the rolling rates (`improving` / `stable` / `declining`). Idempotent re-apply (dedup by `session_id`). | Done (unit-tested) | [src/tompe/services/analytics.py](../src/tompe/services/analytics.py) |
| B4 / blind spots | `detect_blind_spots` cross-products MQM × ToM time series, joins on shared session ids, and flags cells where `min(mqm_rate, tom_rate)` is below `threshold` over `min_sessions`. | Done (unit-tested) | [src/tompe/services/analytics.py](../src/tompe/services/analytics.py) |
| B4 / class analytics | `compute_class_analytics(profiles)` returns per-MQM + per-ToM means, top-5 blind spots ranked by `n_students`, and the class-wide over-editing rate mean. | Done (unit-tested) | [src/tompe/services/analytics.py](../src/tompe/services/analytics.py) |
| B4 / store factories | `build_profile_from_store(student_id)` rebuilds a `StudentProfile` from stored responses + scores, fills `BlindSpot.example_item_ids`, persists to `data/profiles/{student_id}.json`. `compute_class_analytics_for_class(student_ids)` chains the rebuild. | Done | [src/tompe/services/analytics.py](../src/tompe/services/analytics.py) |
| B5 / progression | `recommend_next_level` walks the `PROGRESSION_STAGES` ladder gated on BKT mastery (default threshold 0.98 per spec §6.2). `is_level_unlocked(level)` checks the *prerequisite* skills (active_skills of the previous stage) — to unlock SCOUT you've mastered NAVIGATOR's S1+S2; to unlock ANALYST you've added S3+S4; to unlock EXPERT you've also covered S5+S6. `recommend_exercises` ranks items by how many of their errors land in the student's BlindSpot cells. | Done (unit-tested) | [src/tompe/services/progression.py](../src/tompe/services/progression.py) |
| B5 / badge gating | `check_progression_badge` now gates Scout / Analyst / Expert on `is_level_unlocked(student_id, level)`: BKT mastery ≥ 0.98 on every prerequisite skill *and* `n_observations ≥ 6` per skill. A student whose teacher set `allowed_levels=['analyst']` but who hasn't earned BKT mastery does *not* auto-receive the Analyst badge — closes the spec §8.3 gap. | Done (unit-tested) | [src/tompe/services/badges.py](../src/tompe/services/badges.py) |
| B5 / api wiring | `api_get_feedback` calls `update_from_scoring(student_id, scoring)` after every response, so the BKT tracker stays current without the caller knowing. `api_get_progress` now serves Skill Radar values from `bkt_skill_profile` first, falling back to Sprint #5's `aggregate_skill_profile` for skills with no BKT observations. | Done | [src/tompe/services/api.py](../src/tompe/services/api.py) |

### Verification — Sprint #8

- [ ] **BKT convergence (already run).** 20 all-correct observations from `p_mastery=0.10` push p to ≥ 0.99; 20 all-wrong observations from `p_mastery=0.50` drag p below 0.12. ✓
- [ ] **BKT persistence (already run).** Three correct `update_skill` calls leave `n_observations=3`, `n_correct=3` on disk; `get_mastery` reads the same value back. ✓
- [ ] **Progression gating (already run).** Fresh student: only NAVIGATOR unlocked. After mastering S1+S2 (30 correct each): SCOUT unlocked, ANALYST still locked. After mastering S3+S4: ANALYST also unlocked. ✓
- [ ] **Badge gating (already run).** `check_progression_badge('stu_no_bkt', 'scout', ...)` returns None (no mastery). After mastery is achieved, the same call awards the badge once and not again. ✓
- [ ] **Analytics trend detection (already run).** Constant 0.25 rate across 5 sessions → trend=`stable`; rising 0.2 → 1.0 → trend=`improving`; falling 1.0 → 0.2 → trend=`declining`. ✓
- [ ] **Blind-spot crosstab (already run).** A profile with 3 sessions at `accuracy_rate=0.25` AND `1st_machine_rate=0.25` produces exactly one BlindSpot at (accuracy, 1st_machine) with `detection_rate ≈ 0.25`. ✓
- [ ] **Idempotent profile rebuild (already run).** Re-applying the same `session_results` list does not duplicate the time-series entries. ✓
- [ ] **`compute_class_analytics` (already run).** Two profiles sharing a blind spot at (accuracy, 1st_machine) report `n_students=2` for that cell. ✓

### Scope decisions

- **Manual teacher overrides still take priority over BKT.** `StudentAccount.allowed_levels` is still the source of truth for "what levels can this student start?". BKT gates the progression *badges* (and `recommend_next_level`), but doesn't itself flip `allowed_levels`. Teachers can promote a student manually before BKT signals mastery.
- **Fallback to detection-rate pooling.** When a skill has zero BKT observations, `api_get_progress` falls back to Sprint #5's pooled detection rate so the radar still has some signal during the bootstrap period. As soon as observations exist, BKT takes over for that axis.
- **BKT parameter defaults.** Used the Corbett-Anderson canonical priors (`p_init=0.10`, `p_transit=0.10`, `p_slip=0.10`, `p_guess=0.20`) without per-skill calibration. A future sprint can fit these from pilot data.
...
| ~~B4~~ | ~~**`analytics.py` longitudinal stubs**~~ — **RESOLVED — Sprint #8.** `update_student_profile` / `detect_blind_spots` / `compute_class_analytics` ship with per-MQM and per-ToM `PerformanceTimeSeries`, OLS-slope trend labels, MQM×ToM blind-spot crosstab, and `build_profile_from_store` cache at `data/profiles/`. | Tier C2 | done | — |
| ~~B5~~ | ~~**`progression.py` BKT mastery**~~ — **RESOLVED — Sprint #8.** New `tompe.services.bkt` (Corbett-Anderson update + `data/bkt/` persistence); `recommend_next_level` + `is_level_unlocked` gate Scout / Analyst / Expert on BKT mastery ≥ 0.98 of *prerequisite* skills; `check_progression_badge` honours the gate. `api_get_progress` feeds the Skill Radar from BKT first, falling back to Sprint #5's detection-rate pooling. | Tier C1 | done | — |
...
5. ~~**Longitudinal analytics are all `NotImplementedError`.**~~ **RESOLVED — Sprint #8 (B4).** `update_student_profile` / `detect_blind_spots` / `compute_class_analytics` ship in `services/analytics.py` with per-MQM and per-ToM `PerformanceTimeSeries`, OLS-slope trend labels, MQM×ToM blind-spot crosstab, and a `data/profiles/` cache via `build_profile_from_store`.
6. ~~**BKT mastery + teacher-gated progression is unwired.**~~ **RESOLVED — Sprint #8 (B5).** New `tompe.services.bkt` ships the Corbett-Anderson update + `data/bkt/` persistence; `is_level_unlocked` + `recommend_next_level` in `progression.py` gate on BKT mastery ≥ 0.98 of *prerequisite* skills; `check_progression_badge` now honours the gate so Scout / Analyst / Expert no longer fire on level-int alone.
...
| §3.9 | `StudentProfile` + `BlindSpot` longitudinal model | Implemented | [src/tompe/schemas/scoring.py](../src/tompe/schemas/scoring.py), [src/tompe/services/analytics.py](../src/tompe/services/analytics.py) | Sprint #8 (B4): `update_student_profile` / `detect_blind_spots` / `compute_class_analytics` ship with OLS-slope trends + MQM×ToM crosstab; `data/profiles/` cache via `build_profile_from_store`. |
...
| §2.1 | Scout / Analyst / Expert badges | Implemented | [services/badges.py:63](../src/tompe/services/badges.py#L63), [src/tompe/services/progression.py](../src/tompe/services/progression.py) | Sprint #8 (B5): now gated on `is_level_unlocked(student_id, level)` — BKT mastery ≥ 0.98 of the *prerequisite* skills (n_obs ≥ 6 each). Closes the auto-award-on-level-int gap. |
| §2.2 | BKT mastery (p≥0.98) check for progression | Implemented | [src/tompe/services/bkt.py](../src/tompe/services/bkt.py), [src/tompe/services/progression.py](../src/tompe/services/progression.py) | Sprint #8 (B5): Corbett-Anderson BKT tracker (`p_init=0.10`, `p_transit=0.10`, `p_slip=0.10`, `p_guess=0.20`); per-student JSON at `data/bkt/`. `recommend_next_level` walks the PROGRESSION_STAGES ladder gated on the threshold. |
...
| §6.2 | Axis values from BKT mastery (0.98 threshold) | Implemented | [src/tompe/services/bkt.py](../src/tompe/services/bkt.py), [src/tompe/services/api.py](../src/tompe/services/api.py) | Sprint #5 (B2): `aggregate_skill_profile` shipped. **Sprint #8 (B5)**: `bkt_skill_profile(student_id)` returns per-skill mastery; `api_get_progress` now prefers BKT, falling back to detection-rate pooling only for skills with zero observations. |
...
4. ~~**Progression beyond Navigator is not gated by mastery.**~~ **RESOLVED — Sprint #8 (B5).** Corbett-Anderson BKT tracker + `is_level_unlocked` gate. Scout / Analyst / Expert now require BKT mastery ≥ 0.98 on the prerequisite skills *and* a minimum of 6 observations each, in addition to the teacher's `allowed_levels` unlock.
